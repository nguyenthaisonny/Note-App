...
export const deleteNote = async (params) => {
  console.log(params);
    const query = `mutation Mutation($noteId: String!, $folderId: String!) {
      deleteNote(noteId: $noteId, folderId: $folderId) {
        id
        content
        updatedAt
      }
    }`
    const deleteNote = await graphQLRequest({query, variables: params})
    console.log('[noteUtil]', deleteNote);
    return deleteNote
}
